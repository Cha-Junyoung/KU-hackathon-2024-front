...
const createGradient = (colors: string[]) => {
  ...
  .map((color, index) => `${color} ${(index / colors.length) * 100}%`)
  .join(", ")})`;
};

const GradientCalendarBar = ({ days, diaryList, onClick }: { days: number; diaryList: IDiaryInfo[], onClick: (index:number)=>void }) => {
